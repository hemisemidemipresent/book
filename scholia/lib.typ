...
// ===========================================================================
#let block-head(body) = context block(above: 1em, below: 0.5em, breakable: true, )[
  #text(font: default-fonts.heading, weight: "bold", fill: active.get().thm)[#body]
]

#let trigger(body) = context block(above: 0.5em, below: 0.5em, breakable: true, )[
  #text(font: default-fonts.heading, weight: "bold", fill: active.get().def)[Trigger.] #h(0.3em)#body
]

#let TODO(hint) = context text(font: default-fonts.sans, size: 0.92em, fill: active.get().eg)[[ TODO: #hint ]]
...
#let scholia(theme: "light", prose: "notes", paper: "a4", fonts: none, body) = {
  ...
  // fuck it im making it breakable
